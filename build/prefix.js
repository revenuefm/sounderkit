...
function findExistingPrefix() {
    ...
    allFiles.filter(({file}) => ~file.indexOf('skit.css')).some(({file, data}) => {
                ...
                files.forEach(file =>
                    reads.push(util.read(file, data =>
                        allFiles.push({
                            file,
                            data,
                            replace: (data, needle, replace) => data.replace(new RegExp(`${needle}-`, 'g'), `${replace}-`).replace(new RegExp(`(${needle})?Skit`, 'g'), `${replace === 'sk' ? '' : replace}Skit`)
                        })
                    ))
                );
                Promise.all(reads).then(res);
