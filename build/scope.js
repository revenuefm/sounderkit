...
const currentScopeLegacyRegex = new RegExp('\\.(sk-scope)');
...
if (argv.h || argv.help) {
    console.log(`
        ...
        example:
        scope.js // will scope with sk-scope
...
function getNewScope() {
    ...
    const scopeFromInput = argv.scope || argv.s || 'sk-scope';
            ...
            .then(output => {
                store.data = `/* scoped: ${scopeFromInput} */\n` +
                    output.replace(new RegExp(`.${scopeFromInput} ${/{(.|[\r\n])*?}/.source}`), '')
                        .replace(new RegExp(`.${scopeFromInput}${/\s((\.(sk-(drag|modal-page|offcanvas-page|offcanvas-flip)))|html)/.source}`, 'g'), '$1')
            })
        );
    });
